...
// cp_setGlobalByName safely sets a global variable given the string that defines it.
// --------------------------------------
function cp_setGlobalByName(%global, %value) {
	%firstLetter = getSubStr(%global, 0, 1);
	%theRest = getSubStr(%global, 1, strLen(%global)-1);
	
	if(%firstLetter $= "$") {
		return cp_setGlobalByName(%theRest, %value);
	}
...
// cp_getGlobalByName gives you the value of any global.
// --------------------------------------
function cp_getGlobalByName(%global) {
	%firstLetter = getSubStr(%global, 0, 1);
	%theRest = getSubStr(%global, 1, strLen(%global)-1);
	
	if(%firstLetter $= "$") {
		return cp_getGlobalByName(%theRest);
	}
